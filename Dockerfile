...
RUN cmake .. -DCMAKE_INSTALL_PREFIX=${QTDIR} -DBUILD_SHARED_LIBS=ON -DQT_FEATURE_core=ON -DQT_FEATURE_network=ON -DQT_FEATURE_serialport=ON -DQT_FEATURE_concurrent=ON -DQT_FEATURE_gui=ON -DQT_FEATURE_widgets=ON -DQT_FEATURE_qml=ON -DQT_FEATURE_qtcharts=ON -DQT_FEATURE_qt3d=OFF -DQT_FEATURE_qtmultimedia=OFF -DQT_FEATURE_qtquick3d=OFF -DQT_FEATURE_qtsvg=ON -DQT_FEATURE_qtwebengine=ON -DQT_FEATURE_qtwebsockets=OFF -DQT_FEATURE_qtwayland=OFF -DQT_FEATURE_qttools=ON -DQT_FEATURE_qttranslations=ON -DQT_FEATURE_qtvirtualkeyboard=OFF -DQT_FEATURE_qtwinextras=OFF -DQT_FEATURE_qtxmlpatterns=ON -DQT_FEATURE_qtdeclarative=OFF -DQT_FEATURE_tests=ON -DQT_FEATURE_examples=ON
RUN make -j$(nproc)
RUN make install
