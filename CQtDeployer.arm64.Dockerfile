...
# BASE_IMAGE is passed by the workflow.
...
# Use the same Qt base image for runtime so Qt libs/tools are available.
ARG BASE_IMAGE
...
CMD ["--help"]
```# filepath: c:\Users\yash0\Desktop\user_docker_image\dockerImage\CQtDeployer.arm64.Dockerfile
# Build CQtDeployer on top of the prebuilt Qt 6.7.3 ARM64 image.
# BASE_IMAGE is passed by the workflow.
